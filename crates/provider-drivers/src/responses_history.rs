//! Normalize OpenAI Responses history items shared by Grok and Antigravity.

use provider_core::{ProviderError, ProviderErrorKind};
use serde_json::{Map, Value};

pub(crate) fn reject_unresolved_item_references(
    ...
    for item in input {
        if item.get("type").and_then(Value::as_str) == Some("item_reference") {
            return Err(ProviderError::new(
                ...
                format!(
                    "{provider} continuation state is unavailable to resolve item_reference; resend complete input history"
                ),
            ));
        }
    }
...
pub(crate) fn normalize_input(
    ...
    for mut item in std::mem::take(input) {
        if normalize_input_item(&mut item, provider)? {
...
fn normalize_input_item(item: &mut Value, provider: &str) -> Result<bool, ProviderError> {
    ...
    match item_type.as_str() {
        ...
        "agent_message" => return normalize_agent_message(item_object, provider),
        "custom_tool_call" => {
        ...
        "tool_search_call" => {
            if !non_empty_field(item_object, "call_id") {
                return Err(invalid_request(format!(
                    "{provider} cannot safely replay tool_search_call without a call_id"
                )));
            }
            let call_id = item_object.remove("call_id").unwrap_or(Value::Null);
        ...
        "tool_search_output" => {
            if !non_empty_field(item_object, "call_id") {
                return Err(invalid_request(format!(
                    "{provider} cannot safely replay tool_search_output without a call_id"
                )));
            }
            let call_id = item_object.remove("call_id").unwrap_or(Value::Null);
        ...
        "file_search_call" => {
            reject_hosted_tool_results(item_object, provider, &["results"])?;
            return Ok(convert_hosted_tool_call(
                item_object,
                "file_search",
        ...
        "code_interpreter_call" => {
            reject_hosted_tool_results(item_object, provider, &["outputs"])?;
            return Ok(convert_hosted_tool_call(
                item_object,
                "code_interpreter",
                HostedCallArgs::ActionOrRest,
        ...
        "image_generation_call" => {
            reject_hosted_tool_results(item_object, provider, &["result"])?;
            return Ok(convert_hosted_tool_call(
                item_object,
                "image_generation",
                HostedCallArgs::ActionOrRest,
        ...
        "mcp_tool_call" => {
            if !ensure_call_id(item_object) || !non_empty_field(item_object, "name") {
                return Err(invalid_request(format!(
                    "{provider} replayed mcp_tool_call requires call_id and name",
                )));
            }
            let call_id = item_object.remove("call_id").unwrap_or(Value::Null);
        ...
        "mcp_tool_call_output" => {
            if !non_empty_field(item_object, "call_id") {
                return Err(invalid_request(format!(
                    "{provider} replayed mcp_tool_call_output requires a call_id",
                )));
            }
            let call_id = item_object.remove("call_id").unwrap_or(Value::Null);
            let output = item_object.remove("output").unwrap_or(Value::Null);
        ...
        | "context_compaction" => return Err(unsupported_history_item(provider, &item_type)),
...
fn normalize_agent_message(
...
) -> Result<bool, ProviderError> {
    {
        let content = item_object
            ...
            .ok_or_else(|| {
                invalid_request(format!("{provider} agent_message requires content items"))
            })?;
        ...
        for mut part in std::mem::take(content) {
            let part_object = part.as_object_mut().ok_or_else(|| {
                invalid_request(format!(
                    "{provider} agent_message content items must be objects"
                ))
            })?;
            match part_object.get("type").and_then(Value::as_str) {
                Some("input_text") => {
                    if !part_object.get("text").is_some_and(Value::is_string) {
                        return Err(invalid_request(format!(
                            "{provider} agent_message text must be a string"
                        )));
                    }
                ...
                Some("encrypted_content") => {
                    let Some(text) = part_object
                        ...
                        return Err(invalid_request(format!(
                            "{provider} agent_message encrypted_content must be a string",
                        )));
                    };
                ...
                Some(content_type) => {
                    return Err(invalid_request(format!(
                        "{provider} cannot replay agent_message content type `{content_type}`"
                    )));
                }
                None => {
                    return Err(invalid_request(format!(
                        "{provider} agent_message content requires a type",
                    )));
                }
            }
        }
...
fn reject_hosted_tool_results(
    ...
    if let Some(field) = result_fields.iter().find(|field| {
        ...
        return Err(invalid_request(format!(
            "{provider} cannot preserve `{field}` result semantics from `{item_type}` history"
        )));
    }
    Ok(())
}
...
fn unsupported_history_item(provider: &str, item_type: &str) -> ProviderError {
    invalid_request(format!(
        "{provider} cannot safely replay Responses history item type `{item_type}`"
    ))
}
...
pub(crate) fn item_call_id(item: &Map<String, Value>) -> Option<&str> {
    item.get("call_id")
        .and_then(Value::as_str)
        .map(str::trim)
