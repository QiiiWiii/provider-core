...
impl ClaudeEventConverter {
    ...
    fn convert(&mut self, event: &Value) -> Vec<Bytes> {
            ...
            .unwrap_or_default()
        {
            ...
            "response.failed" => {
                output.push(claude_response_error(event, "Upstream response failed"))
            }
            "response.canceled" | "response.cancelled" => output.push(claude_response_error(
                event,
                "Upstream response was canceled",
            )),
            _ => {}
        }
        output
...
fn claude_error(event: &Value) -> Bytes {
    ...
    claude_error_payload(error, event.get("message").and_then(Value::as_str))
}

fn claude_response_error(event: &Value, default_message: &str) -> Bytes {
    let error = event
        .get("response")
        .and_then(|response| response.get("error"))
        .unwrap_or(&Value::Null);
    claude_error_payload(error, Some(default_message))
}

fn claude_error_payload(error: &Value, default_message: Option<&str>) -> Bytes {
